...
def train(data_file, sheet_name, k_fold, miss_ratio, n_neighbors, chose_featrue):
    ...
    if k_fold > 0:
        ...
        for train_index, vali_index in kf.split(x_train):
            ...
            model = tree.DecisionTreeClassifier()  # Each package provides an untrained model
            model.fit(train_X, train_y)  # Model fitting
            pred = model.predict(vali_X)  # Model validation
    ...
    joblib.dump(model, r'../parameter/tree-{}-{}-{}-{}.pkl'.format(
    ...
    gbdt_log = r'../result/tree_{}_log.txt'.format(state)
    ...
    if state == 'magma':
        plot_confusion_matrix(y_true=y_test, y_pred=y_pred,
                              savename=r"../result/Confusion_Matrix_tree_{}.png".format(state),
                              title="Confusion_Matrix_tree_{}".format(state),
...
if __name__ == '__main__':
    ...
    print(f'tree use {sheet_name}, k_fold {k_fold}, miss_ratio {miss_ratio}')
