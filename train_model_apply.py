...
def train(data_file, sheet_name, k_fold, miss_ratio, n_neighbors, chose_featrue, normal_method, impute_method, upsample_method, usehyparams_search, feat_importance_method, top_k):
    ...
    if feat_importance_method == 'modelbased':
        ...
        if sheet_name == 'Igneous Rocks Database':
            top_k_features = [0, 1, 2, 4, 5, 6, 10, 18]
        else:
            top_k_features = [0, 1, 2, 4, 5, 6, 10, 18] # 这个还要填列的位置
        ...
        if sheet_name == 'Igneous Rocks Database':
            ...
            top_k_features = [0, 1, 2, 3, 4, 5, 8, 16]
    ...
    if usehyparams_search:
        ...
        search_res_df.to_csv(f'parameter/{sheet_name}-search_res.csv', index=False)
    ...
    joblib.dump(model, r'parameter/RandomForest-{}-{}-{}-{}-{}-{}-{}.pkl'.format(
        sheet_name, normal_method, impute_method, upsample_method, usehyparams_search, feat_importance_method,
        top_k))
    ...
    gbdt_log = 'result/Random_forest_{}_log.txt'.format(state)
    ...
    if state == 'magma':
        plot_confusion_matrix(y_true=y_test, y_pred=y_pred,
                              savename="result/Confusion_Matrix_Random_forest_{}.png".format(state),
